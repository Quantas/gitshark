...
public class GitServerReceivePack extends ReceivePack implements PreReceiveHook, PostReceiveHook {
	...
	public void onPreReceive(final ReceivePack rp, final Collection<ReceiveCommand> commands) {
		commands.forEach(command -> {
			...
			case CREATE:
				LOG.info("{} - CREATE {} by {}", repo.getFullDisplayName(), command.getNewId().getName(), user.getUsername());
				break;
			case DELETE:
				LOG.info("{} - DELETE {} by {}", repo.getFullDisplayName(), command.getOldId().getName(), user.getUsername());
				break;
			case UPDATE:
				LOG.info("{} - UPDATE Old={}, New={} by {}", repo.getFullDisplayName(), command.getOldId().getName(), command.getNewId().getName(), user.getUsername());
				break;
			case UPDATE_NONFASTFORWARD:
				LOG.info("{} - UPDATE NON FF Old={}, New={} by {}", repo.getFullDisplayName(), command.getOldId().getName(), command.getNewId().getName(), user.getUsername());
				break;
			default:
				break;
			}
